...
case class SkypeMessage(id: Int,
                        author: String,
                        authorName: String,
                        message: String,
                        sent: Date,
...
class SkypeDb(path: String) extends AutoCloseable {
  ...
  def getMessages(conversationId: Int): Iterator[SkypeMessage] = {
    val messagesStmt = db.prepareStatement("select id, convo_id, author, from_dispname, timestamp, edited_timestamp, body_xml from Messages where convo_id = ? order by timestamp")
    messagesStmt.setInt(1, conversationId)
    iterator(messagesStmt.executeQuery()) map {
      rs ⇒
        SkypeMessage(
          rs.getInt("id"),
          rs.getString("author"),
